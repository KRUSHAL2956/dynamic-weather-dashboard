...
const CONFIG = {
    // API key management - SECURE: Use environment variable only
    get OPENWEATHER_API_KEY() {
        // SECURITY FIX: Prevent timing attacks by always returning same type
        const envKey = (typeof process !== 'undefined' && process.env?.OPENWEATHER_API_KEY);
        const hasValidKey = envKey && envKey.length > 20 && envKey !== 'YOUR_API_KEY_HERE';
        
        if (!hasValidKey) {
            // Return a consistent placeholder to prevent timing attacks
            return 'INVALID_KEY_CONFIGURED';
        }
        
        return envKey;
    },
    
    // SECURITY: Validate API key availability without exposing it
    isApiKeyValid() {
        const key = this.OPENWEATHER_API_KEY;
        return key !== 'INVALID_KEY_CONFIGURED' && key.length > 20;
    },
    
    // API endpoints - SECURITY: Hardcoded trusted URLs only
    ...
    // Content Security Policy - SECURITY: XSS protection
    CSP_POLICY: "default-src 'self'; script-src 'self' 'unsafe-inline' https://unpkg.com https://cdnjs.cloudflare.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdnjs.cloudflare.com; img-src 'self' data: https://openweathermap.org; connect-src 'self' https://api.openweathermap.org; font-src 'self' https://fonts.gstatic.com;",
